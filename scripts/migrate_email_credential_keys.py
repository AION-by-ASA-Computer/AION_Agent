    ...
    async with get_async_session_maker()() as session:
        rows = (
            await session.execute(
                select(UserMcpCredential).where(UserMcpCredential.server_slug == server_slug)
            )
        ).scalars().all()
        for row in rows:
            ...
            print(f"{'would move' if dry_run else 'move'} {row.user_id}: {row.credential_key} -> {new_key}")
            if not dry_run:
