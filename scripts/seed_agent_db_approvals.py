...
async def seed_approval_rules():
    ...
    async with engine.begin() as conn:
        ...
        res = await conn.execute(text("SELECT name FROM sqlite_master WHERE type='table' AND name='approval_rules'"))
        if not res.fetchone():
            print("Table 'approval_rules' not found. Ensure DB bootstrap is done.")
            return

        rules = [
            ('default', 'agent_db_drop_table', '.*', 'ask', 'admin', 'Operazione potenzialmente distruttiva: richiede conferma esplicita dell\'utente'),
            ('default', 'agent_db_alter_table', '.*"operation":\\s*"DROP_COLUMN".*', 'ask', 'admin', 'Rimozione colonna può causare perdita dati: richiede conferma'),
            ('default', 'agent_db_create_table', '.*', 'allow', 'admin', 'Creazione tabella non è distruttiva'),
            ...
            ('default', 'agent_db_alter_table', '.*"operation":\\s*"ADD_COLUMN".*', 'allow', 'admin', 'Aggiunta colonna non distruttiva'),
        ]
