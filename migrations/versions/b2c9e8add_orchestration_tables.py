...
def upgrade() -> None:
    ...
    if "execution_plans" not in names:
        ...
        op.create_index(
            op.f("ix_execution_plans_session_id"),
            "execution_plans",
            ["session_id"],
            unique=False,
        )
        op.create_index(
            op.f("ix_execution_plans_status"),
            "execution_plans",
            ["status"],
            unique=False,
        )
    if "orchestration_audit" not in names:
        ...
        op.create_index(
            op.f("ix_orchestration_audit_plan_id"),
            "orchestration_audit",
            ["plan_id"],
            unique=False,
        )
...
def downgrade() -> None:
    op.drop_index(
        op.f("ix_orchestration_audit_plan_id"), table_name="orchestration_audit"
    )
