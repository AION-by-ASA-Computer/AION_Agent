...
def upgrade() -> None:
    ...
    if "draft_markdown" not in cols:
        op.add_column(
            "execution_plans", sa.Column("draft_markdown", sa.Text(), nullable=True)
        )
    if "approved_markdown" not in cols:
        op.add_column(
            "execution_plans", sa.Column("approved_markdown", sa.Text(), nullable=True)
        )
    if "annotations" not in cols:
        op.add_column(
            "execution_plans", sa.Column("annotations", sa.Text(), nullable=True)
        )
