...
**`session_sandbox`:** `sandbox_list_files`, `sandbox_read_text_file`, `sandbox_write_workspace_file` (solo `workspace/`), `sandbox_execute_python` (exec in-process con **import whitelist**), **`sandbox_install_python_packages`** (pip o `uv` nel **venv per sessione** `data/sessions/<id>/.venv`; abilitato di default, disabilitabile con `AION_SANDBOX_ALLOW_PACKAGE_INSTALL=0`), `sandbox_run_python_file` (solo `workspace/*.py`), **`sandbox_run_node_file`** (`workspace/*.js` per docx-js; richiede Node sul server / `AION_NODE_PATH`) — subprocess con **cwd = root sessione**; di default usa il Python del venv sessione se `AION_SANDBOX_AUTO_VENV=1`, così valgono i pacchetti installati lì; senza venv resta l’interprete del processo MCP). Isolamento: **`AION_SANDBOX_BACKEND=subprocess`** (env scrub) in dev, **`container`** (Podman per sessione, vedi [Session isolation](../security/session-isolation.md)) in produzione. **Rischi:** installazione implica rete e supply chain; in produzione lasciare `AION_SANDBOX_ALLOW_PACKAGE_INSTALL=0` se non serve. Per flussi complessi: scrivere il `.py` con il tool di write ed eseguire con run file.
