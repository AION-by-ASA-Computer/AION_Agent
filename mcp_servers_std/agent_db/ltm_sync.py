...
class AgentDBLTMSync:
    ...
    async def notify_schema_change(self, user_id: str, schema_name: str, table_name: str, operation: str, summary: Dict[str, Any]):
        """Notifies MemPalace of a schema change."""
        if not self._mempalace_client:
            print(f"[LTM Sync Mock] Schema change: {schema_name}.{table_name} ({operation})")
            return
            
        content = self._format_schema_summary(schema_name, table_name, summary)
        try:
            await self._mempalace_client.call("mempalace_write_drawer", {
    ...
    def _format_schema_summary(self, schema_name: str, table_name: str, summary: Dict[str, Any]) -> str:
