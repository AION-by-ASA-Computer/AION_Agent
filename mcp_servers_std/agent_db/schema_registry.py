...
class SchemaRegistry:
    ...
    def register_table(self, schema_name: str, table_name: str, physical_name: str, description: str):
    ...
    def register_columns(self, schema_name: str, table_name: str, columns: List[Dict[str, Any]]):
        cursor = self.conn.cursor()
        # Clean existing columns for this table to avoid duplicates or stale data
        cursor.execute("DELETE FROM _aion_schema_columns WHERE schema_name = ? AND table_name = ?", (schema_name, table_name))
    ...
    def log_history(self, operation: str, schema_name: str, table_name: str, payload: str, conversation_id: str = None, rows_affected: int = 0, rollback_sql: str = None):
        cursor = self.conn.cursor()
        cursor.execute("""
            INSERT INTO _aion_schema_history (schema_name, table_name, operation, payload, conversation_id, rows_affected, rollback_sql)
            VALUES (?, ?, ?, ?, ?, ?, ?)
        """, (schema_name, table_name, operation, payload, conversation_id, rows_affected, rollback_sql))
        self.conn.commit()
    ...
    def describe_table(self, schema_name: str, table_name: str) -> Optional[Dict[str, Any]]:
        cursor = self.conn.cursor()
        cursor.execute("""
