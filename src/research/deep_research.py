...
class DeepResearcher:
    ...
    async def _llm(self, messages: List[Dict], temperature: float = 0.3,
        ...
        """Call the AION LLM asynchronously."""
    ...
    async def _search(self, query: str) -> List[Dict]:
        """Run a search query via AION native web_search."""
        try:
            results = await search_web(query, max_results=10)
            if results:
                prov = results[0].get("provider") or "aion"
                if prov not in self.providers_used:
                    self.providers_used.append(prov)
                logger.info("Research search returned %d results for %r", len(results), query[:80])
                return results
            self._last_search_error = "no results from web_search"
            return []
        except Exception as e:
    ...
    async def _fetch_and_extract(self, url: str, question: str,
        ...
        self._emit(phase="reading", url=url, title=display,
                   total_sources=len(self.urls_fetched))
        try:
            page = await fetch_webpage_content(url)
        except Exception as e:
