...
async def run_evaluation(dataset_path: str, profile_name: str, threshold: float):
    ...
    print(f"Avvio Evaluation Run: {run_id} | Dataset: {dataset_path} | Profilo: {profile_name}")
    ...
    for idx, case in enumerate(cases):
        ...
        agent_instance, p_name = await get_agent(profile_name, session_id=session_id, user_id="eval")
        pipeline = AgentPipeline(agent_instance, session_id=session_id, profile_name=p_name, user_id="eval")
...
def main():
    ...
    parser.add_argument("--dataset", type=str, required=True, help="Path al file JSON del dataset")
    parser.add_argument("--profile", type=str, default="AION Core", help="Profilo agente da valutare")
    parser.add_argument("--threshold", type=float, default=0.8, help="Soglia minima di successo (0.0 - 1.0) per passare la CI")
    ...
    success = loop.run_until_complete(run_evaluation(args.dataset, args.profile, args.threshold))
