...
async def evaluate_with_llm_judge(case: dict, actual_output: str) -> Tuple[float, str]:
    ...
    agent_instance, profile_name = await get_agent("LLM Judge", session_id="eval_judge", user_id="eval")
    pipeline = AgentPipeline(agent_instance, session_id="eval_judge", profile_name=profile_name, user_id="eval")
