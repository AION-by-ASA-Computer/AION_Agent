...
class TimelineBuilder:
  ...
  def apply_chunk(self, chunk: Dict[str, Any]) -> None:
    ...
    if ctype == "token":
      ...
      filtered, self._plan_capture_phase, self._plan_capture_pending = feed_plan_aware_display(
    ...
    if ctype == "tool_event":
      ...
      if et == "request_sync":
        ...
        if tid not in self._tool_steps:
          ...
          self._tool_steps[tid] = {"id": tid, "name": sync_name, "input": {"_pending": True}, "status": "running"}
          if tid not in self._tool_order:
            self._tool_order.append(tid)
          self._active_tool_by_name[sync_name] = tid
      elif et == "tool_start":
        tid = str(ev.get("id") or "").strip() or f"{name}:{len(self._tool_order)}"
        self._upsert_tool(
          {
            "kind": "tool",
            "id": tid,
        ...
        self._tool_steps[tid] = {"id": tid, "name": name, "input": ev.get("input"), "status": "running"}
        if tid not in self._tool_order:
          self._tool_order.append(tid)
        ...
        cur = self._tool_steps.get(tid) or {"id": tid, "name": name, "input": ev.get("input") or {}}
        output = str(ev.get("output") or "")
        seg = {
          "kind": "tool",
          "id": tid,
          "name": cur.get("name") or name,
          "input": cur.get("input") if cur.get("input") is not None else ev.get("input"),
          "output": output,
          "status": "done",
        }
    ...
    if ctype == "artifact_start":
      ...
      idx = next((i for i, s in enumerate(self.segments) if s.get("kind") == "artifact" and s.get("id") == aid), -1)
      if idx >= 0:
    ...
    if ctype == "artifact_content":
      ...
      ids = [str(aid)] if aid and str(aid) in self._artifacts else list(self._artifact_order)
      ...
      for art_id in ids:
        ...
        idx = next(
          (i for i, s in enumerate(self.segments) if s.get("kind") == "artifact" and s.get("id") == art_id),
          -1,
        )
        if idx >= 0:
    ...
    if ctype == "artifact_end":
      ...
      self._artifacts[aid] = {
        **cur,
        ...
        "execution": str(art["execution"]) if art.get("execution") is not None else None,
      }
      idx = next((i for i, s in enumerate(self.segments) if s.get("kind") == "artifact" and s.get("id") == aid), -1)
      if idx >= 0:
        self.segments[idx] = {
  ...
  def _append_reasoning(self, piece: str) -> None:
    ...
    self.segments.append(
      {"kind": "reasoning", "id": _seg_id("reasoning", len(self.segments)), "content": piece}
    )
  ...
  def _append_text(self, piece: str) -> None:
    ...
    if self.segments and self.segments[-1].get("kind") == "text":
      ...
      self.segments[-1] = {**last, "content": (last.get("content") or "") + piece}
      return
    self.segments.append({"kind": "text", "id": _seg_id("text", len(self.segments)), "content": piece})

  def _upsert_tool(self, seg: Dict[str, Any]) -> None:
    tid = seg.get("id")
    idx = next((i for i, s in enumerate(self.segments) if s.get("kind") == "tool" and s.get("id") == tid), -1)
    if idx >= 0:
