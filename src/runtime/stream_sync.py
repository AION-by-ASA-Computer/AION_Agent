...
class StreamSync:
    ...
    def wait_for_sync(cls, session_id: str, timeout: float = 10.0):
        ...
        evt = cls.get_event(session_id)
        if not evt.is_set():
            logger.info("SYNC: Tool in session %s waiting for stream catch-up (timeout=%s)...", session_id, timeout)
            start = time.time()
            if not evt.wait(timeout):
                logger.warning("SYNC: Timeout waiting for stream sync in session %s after %ss", session_id, time.time()-start)
            else:
                logger.info("SYNC: Session %s sync OK after %ss", session_id, time.time()-start)
    ...
    def purge(cls, session_id: str):
        ...
        with cls._lock:
            if session_id in cls._events:
                ...
                try:
                    cls._events[session_id].set()  # Free any waiting thread before purging
                except Exception:
                    pass
