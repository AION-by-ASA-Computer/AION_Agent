...
class ArtifactManager:
    ...
    def save(self, identifier: str, content: str, 
             artifact_type: str, filename: Optional[str] = None) -> Tuple[Path, int]:
        """
    ...
    def _get_next_version_num(self, target_name: str) -> int:
        ...
        if archive_dir.exists():
            existing_versions = list(archive_dir.glob(f"v*{os.path.splitext(target_name)[1]}"))
            if existing_versions:
                ...
                for v_file in existing_versions:
                    ...
                    if m: v_nums.append(int(m.group(1)))
                if v_nums:
