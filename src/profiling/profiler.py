...
class PerTurnProfiler:
    def __init__(self):
        ...
        self._active_profiles = defaultdict(
            lambda: {
                "timestamp": None,
                "total_seconds": 0,
        ...
        if start is not None:
            ...
            if phase_name == "tool_total" and custom_data:
                prof["tool_per_call"].append(
                    {"name": custom_data.get("name", "unknown"), "duration": duration}
                )
    ...
    def finish_turn(self, ctx):
        ...
        if self.storage == "jsonl":
            ...
            record = {"conversation_id": key[0], "user_id": key[1], **prof}
            try:
...
async def _prof_post_tool(ctx):
    profiler.end_phase(
        ctx, "tool_total", custom_data={"name": ctx.payload.get("tool_name")}
    )
