...
class GrafanaTools:
    ...
    def create_dashboard(self, dashboard_name: str, promql: str, panel_type: str = "timeseries") -> str:
        url = f"{self.url}/api/dashboards/db"
        cleaned_promql = re.sub(r'\[[0-9]+[smhdwy](:[0-9]+[smhdwy])?\]$', '', promql.strip())
        uid = dashboard_name.replace(" ", "_").lower()
        
        target_json = {"expr": cleaned_promql, "legendFormat": "{{instance}}"}
        if panel_type == "gauge": target_json["instant"] = True
        ...
        if panel_type == "gauge":
            panel_config["options"] = {"reduceOptions": {"values": True, "calcs": ["last"]}}

        payload = {
            "dashboard": {
                "id": None, "uid": uid, "title": dashboard_name,
                "panels": [panel_config], "schemaVersion": 36, "version": 1
            },
            "overwrite": True
        }
    ...
    def update_dashboard(self, dashboard_uid: str, new_title: str, promql: str) -> str:
        ...
        try:
            ...
            if not dashboard: return "Dashboard JSON malformed."
            ...
            update_resp = requests.post(update_url, json={"dashboard": dashboard, "overwrite": True}, headers=self._headers())
