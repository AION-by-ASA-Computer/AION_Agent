...
class PrometheusTools:
    ...
    def search_metric(self, metric_name: str) -> str:
        try:
            ...
            if data.get("status") == "success":
                ...
                if len(matched) > 50:
                    return (
                        f"Found {len(matched)} metrics (showing first 50):\n"
                        + "\n".join(matched[:50])
                    )
    ...
    def get_chart_data(self, query: str) -> Optional[ChartData]:
        ...
        for item in result_data:
            ...
            time_index = pd.to_datetime(timestamps, unit="s", utc=True).tz_convert(
                "Europe/Rome"
            )
    ...
    def _format_vector_result(self, result_data: list) -> str:
        ...
        for item in result_data:
            labels = {
                k: v for k, v in item.get("metric", {}).items() if k != "__name__"
            }
    ...
    def _format_matrix_stats(self, result_data: list) -> str:
        ...
        for item in result_data:
            ...
            if metric_values:
                label_str = self._extract_label_string(item.get("metric", {}))
                stats_lines.append(
                    f"- {label_str}: Curr={metric_values[-1]:.2f}, Avg={sum(metric_values) / len(metric_values):.2f}"
                )
