...
def setup_logging():
    ...
    def add_otel_context(_, __, event_dict):
        ...
        if span.is_recording():
            ...
            if attrs:
                for k in ("aion.profile", "aion.session_id", "aion.user_id", "aion.tenant_id", "tool.name", "tool.mcp_server", "tool.output", "tool.error", "file.name", "extract.status"):
                    if k in attrs:
                        val = attrs[k]
                        event_dict[k] = val
                        clean_key = k.split(".")[-1]
    ...
    for name in list(logging.root.manager.loggerDict.keys()):
        ...
        if name.startswith("aion") or name.startswith("src") or name.startswith("agent"):
