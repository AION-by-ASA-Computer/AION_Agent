...
def check_prompt(profilo: str, contesto: str, user_input: str) -> tuple:
    ...
    prompt_formattato = prompt_obj.format(
        profilo=profilo, contesto=contesto, user_input=user_input
    )
...
def call_llm_api(prompt_text: str, model_config: dict) -> str:
    ...
    try:
        ...
        logger.warning(
            f"Chiamata LLM fallita o non disponibile ({e}). Esecuzione del Fallback Mock..."
        )
        ...
        update_current_span(
            ...
            metadata={
                "warning": "chiamata fallita, utilizzato fallback mock",
                "error_dettagliato": str(e),
            },
        )
...
def execute_tool_ricerca(db_table: str, search_criteria: dict) -> list:
    ...
    filtered_results = [
        r for r in results if r["stato"] == search_criteria.get("stato", "disponibile")
    ]

    # Aggiorna lo span con il numero di record trovati prima di ritornare
    update_current_span(metadata={"records_returned": len(filtered_results)})
...
def skill_ricerca_db(query_str: str) -> dict:
    ...
    update_current_span(
        metadata={"skill_class": "DatabaseCapability", "query_originale": query_str}
    )
    ...
    update_current_span(
        metadata={"records_count": risultato["numero_record"], "ricerca_avvenuta": True}
    )
...
def skill_calcolo_metrico(dati_input: list) -> dict:
    ...
    update_current_span(
        metadata={"metric_score": risultato["valore"], "calcolo_completato": True}
    )
...
def run_agent(
    user_input: str, active_profile: str = "Tecnico_Senior", session_id: str = None
) -> str:
    """
    ...
    """
    logger.info(
        f"Avvio Agent per input utente. Profilo attivo: {active_profile} | Sessione/Thread: {session_id}"
    )
    ...
    model_config = {"model": _model_name, "temperature": 0.4, "max_tokens": 1024}
    ...
    prompt_formattato, prompt_originale = check_prompt(
        profilo=active_profile, contesto=contesto_simulato, user_input=user_input
    )
...
if __name__ == "__main__":
    ...
    try:
        ...
        risposta_1 = run_agent(
            test_input_1, active_profile=profilo_test, session_id=session_id
        )
        ...
        risposta_2 = run_agent(
            test_input_2, active_profile=profilo_test, session_id=session_id
        )
    ...
    print(
        "   - All'interno puoi vedere il template con i placeholder {{profilo}}, {{user_input}}."
    )
    print(
        "   - Ad ogni esecuzione, Opik tiene traccia del commit/versione del prompt usato."
    )
    ...
    print(
        "   - Nei METADATA vedrai le impostazioni del modello (temperatura, max_tokens) e il profilo."
    )
    print("\n3. SEZIONE 'Threads':")
    print("   - Troverai una sessione di chat / thread con ID associato.")
    print(
        "   - Potrai visualizzare la conversazione completa su più turni e il relativo feedback score."
    )
    ...
    print(
        "     │    └── execute_tool_ricerca (Span di tipo TOOL con input/output del DB)"
    )
    print("     ├── skill_calcolo_metrico (Span della Skill di calcolo)")
    print(
        "     └── call_llm_api (Span di tipo LLM con token, parametri ed input/output del modello)"
    )
    print("=" * 80 + "\n")
