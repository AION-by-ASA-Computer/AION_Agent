...
async def test_resolve_active_plan_id_prefers_approved(monkeypatch):
    ...
    monkeypatch.setattr(
        "src.runtime.orchestration_db.list_plans_for_session", fake_list
    )
...
async def test_mark_task_completed_uses_active_plan_when_plan_id_empty(monkeypatch):
    ...
    monkeypatch.setattr(
        "src.runtime.orchestration_db.fetch_plan_session", fake_fetch_session
    )
    monkeypatch.setattr(
        "src.runtime.orchestration_db.fetch_plan_record", fake_fetch_record
    )
    monkeypatch.setattr(
