...
def test_chat_ui_history_preserves_steps_artifacts_and_reasoning(monkeypatch, tmp_path):
    async def run():
        ...
        await bridge.add_message("conv-1", "user", "Crea uno script", user_id="u1", message_id="user-1")
        ...
        tool_kinds = [s["kind"] for s in assistant["timeline"] if s["kind"] in ("tool", "artifact")]
...
def test_chat_ui_history_shows_empty_assistant_with_steps(monkeypatch, tmp_path):
    async def run():
        await _reset_unified_db(monkeypatch, tmp_path)
        bridge = UnifiedHistoryBridge()
        await bridge.add_message("conv-empty", "user", "Run tool", user_id="u1", message_id="user-1")
        await bridge.add_message(
            "conv-empty",
            "assistant",
            "",
        ...
        payload = await get_conversation_messages_chat_ui("conv-empty", x_aion_user_id="u1")
...
def test_chat_ui_history_shows_terminal_empty_assistant(monkeypatch, tmp_path):
    async def run():
        await _reset_unified_db(monkeypatch, tmp_path)
        bridge = UnifiedHistoryBridge()
        await bridge.add_message("conv-term", "user", "Ciao", user_id="u1", message_id="user-1")
        await bridge.add_message(
            "conv-term",
            "assistant",
            "",
        ...
        payload = await get_conversation_messages_chat_ui("conv-term", x_aion_user_id="u1")
        assert len(payload["messages"]) == 2
...
def test_chat_ui_history_404_for_wrong_user(monkeypatch, tmp_path):
    async def run():
        await _reset_unified_db(monkeypatch, tmp_path)
        bridge = UnifiedHistoryBridge()
        await bridge.add_message("conv-own", "user", "Hi", user_id="owner", message_id="user-1")
...
def test_chat_ui_history_attaches_orphans_only_to_next_assistant(monkeypatch, tmp_path):
    async def run():
        await _reset_unified_db(monkeypatch, tmp_path)
        bridge = UnifiedHistoryBridge()
        await bridge.add_message("conv-2", "user", "Prima domanda", user_id="u1", message_id="user-1")
        await bridge.add_step("conv-2", name="legacy_tool", type="tool", output="legacy")
        await bridge.add_attachment(
            "conv-2",
        ...
        await bridge.add_message("conv-2", "user", "Seconda domanda", user_id="u1", message_id="user-2")
        await bridge.add_message("conv-2", "assistant", "Risposta", user_id="u1", message_id="assistant-1")
...
def test_attachment_persistence_is_idempotent(monkeypatch, tmp_path):
    async def run():
        await _reset_unified_db(monkeypatch, tmp_path)
        bridge = UnifiedHistoryBridge()
        await bridge.add_message("conv-3", "user", "Ciao", user_id="u1", message_id="user-1")
        await bridge.add_message("conv-3", "assistant", "Fatto", user_id="u1", message_id="assistant-1")
        for _ in range(2):
            await bridge.add_attachment(
                "conv-3",
...
def test_chat_ui_history_get_last_assistant_steps(monkeypatch, tmp_path):
    async def run():
        ...
        await bridge.add_message("conv-steps", "user", "Ciao", user_id="u1", message_id="user-1")
        await bridge.add_message("conv-steps", "assistant", "Ecco", user_id="u1", message_id="assistant-1")
        await bridge.add_step(
            "conv-steps",
            name="read_file",
            type="tool",
        ...
        await history_mgr.add_message("conv-fallback", "user", "Ciao", user_id="u1", message_id="user-fb-1")
        await history_mgr.add_message("conv-fallback", "assistant", "Ecco", user_id="u1", message_id="assistant-fb-1")
        ...
        async with aiosqlite.connect(str(db_file)) as db:
            ...
            await db.execute(
                ...
                "VALUES (?, ?, ?, ?, ?, ?, 0)",
                ("conv-fallback", "temp-id", "execute_command", "tool", '{"cmd":"ls"}', "file1\nfile2")
            )
            await db.commit()
            
            db.row_factory = aiosqlite.Row
            async with db.execute("SELECT id FROM messages WHERE session_id = 'conv-fallback' AND role = 'assistant'") as cur:
