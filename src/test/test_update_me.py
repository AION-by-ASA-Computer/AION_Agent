...
def test_update_me_endpoint(tmp_sqlite_url: str, monkeypatch) -> None:
    async def run() -> None:
        ...
        body1 = UpdateUserMetadata(display_name="Alice in Wonderland", email="alice@wonderland.com")
        ...
        with pytest.raises(HTTPException) as exc_info:
            await update_me(body=UpdateUserMetadata(identifier="   "), authorization=auth_header)
        ...
        with pytest.raises(HTTPException) as exc_info:
            await update_me(body=UpdateUserMetadata(identifier="bob"), authorization=auth_header)
        assert exc_info.value.status_code == 400
