...
class TestObservabilityMetrics(unittest.TestCase):
    ...
    def test_tool_use_hooks_latency(self):
        ...
        post_ctx = HookContext(
            ...
            user_id="test_user",
            profile="test_profile",
            payload={"tool_name": "test_tool", "server_name": "native", "input": {}, "status": "ok"}
        )
    ...
    def test_mcp_health_metric_dynamic_initialization(self):
        ...
        val = metrics.aion_mcp_server_healthy.labels(mcp_server=test_mcp_name)._value.get()
        ...
        mcp_manager.get_server_config = lambda name: None if name == test_ctx_mcp_name else orig_get_server_config(name)
        ...
        # Check that the metric has been initialized and exists in the registry map
        self.assertIn(label_tuple_ctx, metrics.aion_mcp_server_healthy.prom_metric._metrics)
        val_ctx = metrics.aion_mcp_server_healthy.labels(mcp_server=test_ctx_mcp_name)._value.get()
    ...
    def test_mcp_health_metric_multi_session_and_profile_switch(self):
        ...
        mcp_manager.get_server_config = lambda name: {"type": "stdio", "command": "invalid_command_xyz_123", "args": []} if name in (mcp_a, mcp_b) else orig_get_config(name)
        mcp_manager._is_stdio_server = lambda name: True if name in (mcp_a, mcp_b) else orig_is_stdio(name)
        ...
        async def warm_a():
            ...
            mcp_manager._session_ctx[other_session_id] = ("plane_assistant", "default", "default")
