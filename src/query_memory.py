...
EMBEDDINGS_PROVIDER = (
    (os.getenv("AION_EMBEDDINGS_PROVIDER") or "openai").strip().lower()
)
...
class QueryMemory:
    ...
    def get_embedding(self, text: str) -> Optional[np.ndarray]:
        ...
        try:
            ...
            response = requests.post(
                url, json=payload, headers=headers, timeout=EMBEDDING_REQUEST_TIMEOUT
            )
    ...
    async def search(
        ...
        async with self.session_maker() as session:
            ...
            if query_embedding is not None:
                q = select(CachedQuery).where(
                    CachedQuery.namespace == namespace, CachedQuery.embedding != None
                )
                if verified_only:
                ...
                for cand in candidates:
                    ...
                    score = np.dot(query_embedding, cemb) / (
                        np.linalg.norm(query_embedding) * np.linalg.norm(cemb)
                    )
                    if score > 0.7:
                        results.append(
                            {
                                ...
                                "promql_query": cand.promql_query,
    ...
    async def get_recent(
        self, limit: int = 50, namespace: str = "default"
    ) -> List[Dict[str, Any]]:
        async with self.session_maker() as session:
            q = (
                select(CachedQuery)
                .where(CachedQuery.namespace == namespace)
