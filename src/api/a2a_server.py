...
async def invoke_agent(msg: A2AMessage, x_aion_origin: str = Header(None)):
    ...
    if msg.method != "agent.ask":
        raise HTTPException(status_code=400, detail="Method not supported. Use 'agent.ask'")
    ...
    try:
        ...
        agent_instance, p_name = await get_agent(profile_name, session_id=session_id, user_id=f"a2a_{x_aion_origin or 'external'}")
        
        # 2. Setup pipeline
        pipeline = AgentPipeline(agent_instance, session_id=session_id, profile_name=p_name, user_id="a2a")
