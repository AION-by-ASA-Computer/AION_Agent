...
> **Operational source of truth:** Mnemos project notes (SQL QM project **`alibr`**, not `default`). In chat-ui select project **Alibr** before queries. Read-only skill — prefer `memory_recall` on the active project.
